...
def conectar_banco():
    try:
        ...
        sqlstate = ex.args[0]
        if sqlstate == 'IM002': # Driver não encontrado
            print("ERRO: Driver ODBC do Oracle não encontrado.")
            print("Verifique se o nome do DRIVER na CONNECTION_STRING está correto.")
            print("Certifique-se de ter o Oracle Instant Client (com ODBC) instalado e configurado no PATH.")
        elif ex.args[0] in ('08001', 'NA'): # Erros comuns de conexão Oracle (TNS, Listener, etc.)
            print(f"Erro de conexão com o Oracle: {ex}")
            print("Verifique se o DBQ (host/porta/serviço) e UID/PWD estão corretos.")
        else:
...
def inserir_registros(conn):
    ...
    try:
        if opcao == '1':
            ...
            data_inicio_str = input("Data de início (YYYY-MM-DD) [opcional]: ")
            data_fim_str = input("Data de fim (YYYY-MM-DD) [opcional]: ")

            data_inicio = data_inicio_str if data_inicio_str else None
...
def remover_registros(conn):    
    ...
    if opcao == '1':
        ...
        try:
            ...
            if e.args[0] == '23000' or 'ORA-02292' in str(e):
    ...
    else: # Este 'else' estava no lugar errado no código original
        print("Opção inválida.")
        return
